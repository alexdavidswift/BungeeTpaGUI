...
public class SpigotPluginMessenger extends BungeeTpBridgeMessenger<BungeeTpPlayer> implements PluginMessageListener {
    ...
    @Override
    protected void handleTeleportGuiRequest(MessageOpenTeleportGui request) {
        final BungeeTpPlayer bungeeTpPlayer = this.plugin.getBungeeTpPlayer(request.getTarget());
        if (bungeeTpPlayer == null) {
            return;
        }

        final Player player = (Player) bungeeTpPlayer.getHandle();
        this.plugin.getGuiManager().openTeleportGui(player, request.getPlayers());
    }
